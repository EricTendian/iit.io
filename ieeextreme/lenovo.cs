...
class Solution {
    static void Main(string[] args) {
        ...
        for (var i = 0; i < numIndices; i++) {
            ...
            Console.WriteLine((alignments[index - 1].Select(t => t.Item1).ToArray());
            Console.WriteLine((alignments[index - 1].Select(t => t.Item2).ToArray());
        }
    }
    ...
    static List<List<Tuple<char, char>>> Align(string seq1, string seq2) {
        ...
        return alignments.OrderBy(l => l.Count)
                         .ThenBy(l => new string(l.Select(t => t.Item1).ToArray()))
                         .ThenBy(l => new string(l.Select(t => t.Item2).ToArray())).ToList();
    }
    ...
    static List<List<char>> GoDeeper(List<char> sequence, string str, int position, int left, int length) {
        if (position >= str.Length) return new List<List<char>>{sequence};

        List<List<char>> possibilities = new List<List<char>>();
        for (var i = 0; i <= left; i++) {
            var list = new List<char>();
            list.AddRange(sequence);
            if (position >= 0) list.Add(str[position]);

            for (var x = 0; x < i; x++) list.Add('-');

            var deeper = GoDeeper(list, str, position + 1, left - i, length);
            if (deeper != null) possibilities.AddRange(deeper);
        }
    ...
    static bool IsListInList(List<List<Tuple<char, char>>> haystack, List<Tuple<char, char>> needle) {
        foreach (var l in haystack) {
            if (l.Count != needle.Count) continue;
            if (!l.Where((t, i) => !Equals(t, needle[i])).Any()) return true;
        }
